...
25. 场景-USB/媒体 — USB/U盘加载、音频/图库/媒体播放
"""
...
def parse_response(text: str) -> dict[str, dict]:
    ...
    返回: {bug_id: {"category": str, "confidence": str, "reason": str}}
    """
    results = {}

    # 匹配 Markdown 表格行: | BUGxxx | 分类 | 置信度 | 判断依据 |
    pattern = re.compile(
    ...
    for line in text.split("\n"):
        ...
        category = re.sub(r"^\d+[.\s]+", "", m.group(2).strip()).strip()
        confidence = m.group(3).strip()
        reason = m.group(4).strip()

        # 跳过表头行
        if bug_id.lower() in ("bug id", "bug_id", "---") or "---" in category:
            continue
...
def main():
    ...
    # 筛选需人工判断的
    manual_bugs = [b for b in all_bugs if b.get("root_cause_category") == "需人工判断"]
    if args.max > 0:
        manual_bugs = manual_bugs[:args.max]

    total_manual = len(manual_bugs)
    print(f"\n📊 统计:")
    print(f"   Bug 总数: {len(all_bugs)}")
    print(f"   需人工判断: {total_manual}")
    if args.max > 0:
        print(f"   本次精校: 前 {args.max} 条")

    if args.dry_run:
        print(f"\n🔍 DRY RUN — 仅展示，不调用 API")
        for bug in manual_bugs[:5]:
            print(f"   - {bug.get('bug_id')}: {bug.get('title', '')[:60]}")
        if len(manual_bugs) > 5:
            print(f"   ... 还有 {len(manual_bugs) - 5} 条未展示")
        return
    ...
    batches = [manual_bugs[i:i+batch_size] for i in range(0, total_manual, batch_size)]
    total_batches = len(batches)

    all_results: dict[str, dict] = {}

    print(f"\n🚀 开始分批精校 ({total_batches} 批, 每批 {batch_size} 条)...")

    for i, batch in enumerate(batches, 1):
        print(f"\n--- 第 {i}/{total_batches} 批 ({len(batch)} 条) ---")
        response = call_deepseek(batch, i, total_batches)
    ...
    print(f"📊 精校汇总")
    print(f"{'='*50}")
    print(f"   总提交精校: {total_manual} 条")
    print(f"   成功精校: {total_results} 条")
    ...
    print(f"📈 精校前后对比 (全量)")
    print(f"{'='*50}")
    print(f"   需人工判断: {len(manual_bugs)} → {final_manual}")
    if len(manual_bugs) - final_manual > 0:
        print(f"   成功精校: {len(manual_bugs) - final_manual} 条")
    print(f"\n✅ 精校完成")
